...
import type { NamedNode, BlankNode, Quad, Quad_Object, Quad_Subject, Term } from "@rdfjs/types";
import { DataFactory } from "rdf-data-factory";
import { RdfStore } from "rdf-stores";
import { PROV, RDF, SDS, SHACL } from "@treecg/types";
import { getSubjects, getObjects } from "./utils";
...
export async function getLatestStream(store: RdfStore): Promise<NBNode | undefined> {
    const streams = (await getSubjects(store, RDF.terms.type, SDS.terms.Stream))
        .filter(
...
export async function getLatestShape(streamId: Term, store: RdfStore): Promise<NBNode | undefined> {
    ...
    const shapeIds = (await Promise.all(shapes.map(async (id) => {
...
async function getLatestDataset(streamId: Term, store: RdfStore): Promise<Term | undefined> {
...
export function transformMetadata(
    ...
    return async (quads: Quad[]) => {
        ...
        let datasetId = latest ? await getLatestDataset(latest, store) : undefined;
        ...
        store.addQuad(df.quad(<Quad_Subject>streamId, RDF.terms.type, SDS.terms.Stream));
        if (datasetId) {
        ...
        store.addQuad(df.quad(<Quad_Subject>streamId, SDS.terms.carries, blank));
        store.addQuad(df.quad(
            <Quad_Subject>streamId,
        ...
        store.addQuad(df.quad(blank, RDF.terms.type, df.namedNode(itemType)));

        if (newShape) {
            store.addQuad(df.quad(blank, SDS.terms.shape, <Quad_Object>newShape));
        }
...
export function createProperty(
    ...
    if (minCount !== undefined) {
        store.addQuad(df.quad(newId, SHACL.terms.minCount, df.literal(minCount.toString())));
    }
    if (maxCount !== undefined) {
        store.addQuad(df.quad(newId, SHACL.terms.maxCount, df.literal(maxCount.toString())));
    }
